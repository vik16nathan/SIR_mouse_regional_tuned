...
if __name__ == "__main__":
    ...
    for epicentre in ['CP', 'DG']:
        ...
        for atrophy_map in cp_hipp_atrophy_maps_comparison[epicentre]:
            ...
            command_filename = (
                sir_commands_dir
                + f"{atrophy_map_stem}_{direction}_{conn_suff}_clearance_missing.txt"
            )

            ##load missing clearance genes
            missing_clearance_genes= np.array(pd.read_csv(atrophy_results_dir+"missing_genes.csv")).flatten()


            with open(command_filename, "w") as f:

                for gene in missing_clearance_genes:
    ...
    for epicentre in ['CP', 'CA1']:
        ...
        for pSyn_map in cp_hipp_pSyn_maps_comparison[epicentre]:
            ...
            command_filename = (
                sir_commands_dir
                + f"{pSyn_map_stem}_{direction}_{conn_suff}_clearance_missing.txt"
            )


            missing_clearance_genes= np.array(pd.read_csv(pSyn_results_dir+"missing_genes.csv")).flatten()
